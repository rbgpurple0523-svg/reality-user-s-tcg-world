...
async function moderateCardTexts(texts: string[]): Promise<ModerationResult> {
  ...
  try {
    ...
    result = {
      ...
      code: parsed.code || 'MODERATION_INVALID_RESPONSE',
    };
  } catch {
    result = {
      allowed: false,
...
export default function CardGenerator({ selectedCoordinate, onBackToHub }: CardGeneratorProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const editingKeepsCurrentCoordinate = Boolean(
      editingId &&
        entries.some(
          (entry) =>
            entry.id === editingId &&
            entry.presetId === currentCoordinate.id,
        ),
    );
    ...
    try {
      ...
      if (!moderationResult.allowed) {
        if (
          ...
          moderationResult.code === 'MODERATION_FLAGGED'
        ) {
        ...
        setIsModerating(false);
        setErrorMessage(
          `入力内容を安全確認できなかったため、保存していません。時間をおいてもう一度お試しください。原因コード: ${moderationResult.code}`,
        );
        return;
      }
    } catch {
      setIsModerating(false);
      setErrorMessage(
        '入力内容を安全確認できなかったため、保存していません。時間をおいてもう一度お試しください。原因コード: MODERATION_CLIENT_ERROR',
      );
      return;
    }
    ...
    let creatorToken = editingEntry
      ? creatorTokens[editingEntry.id]
      : undefined;
  ...
  const handleEdit = (entry: EntryRecord) => {
    ...
    const preset =
      COORDINATE_PRESETS.find((item) => item.id === entry.presetId) ?? null;
      ...
      entry.skillVoices?.length === 4
        ...
        : [
            getDefaultSkillVoice(
              restoredSkills[0] || preset?.defaultSkills[0] || '技1',
            ),
            getDefaultSkillVoice(
              restoredSkills[1] || preset?.defaultSkills[1] || '技2',
            ),
            getDefaultSkillVoice(
              restoredSkills[2] || preset?.defaultSkills[2] || '技3',
            ),
            getDefaultSkillVoice(
              restoredSkills[3] || preset?.defaultSkills[3] || '技4',
            ),
          ],
    );
    ...
    <div className="max-w-5xl mx-auto p-6 bg-white rounded-2xl border border-gray-200 shadow-lg space-y-8 text-gray-900">
      <div className="flex justify-between items-center border-b pb-4 gap-4">
        ...
        {onBackToHub && (
          <button
            type="button"
            onClick={onBackToHub}
            className="text-xs font-bold px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg"
          >
            ← 一覧に戻る
          </button>
        )}
        ...
        <div className="p-4 rounded-xl border border-amber-200 bg-amber-50 flex flex-wrap items-center justify-between gap-3 text-xs">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-3 py-2 rounded-lg bg-amber-600 text-white font-bold"
            >
              続きから作成する
            </button>
            ...
            <button
              ...
              className="px-3 py-2 rounded-lg bg-white border border-amber-300 font-bold text-amber-900"
            >
              破棄
            </button>
          </div>
        ...
        <div className="space-y-5">
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            ...
            <div className="pt-3 border-t border-gray-100 space-y-4">
              <div>
                ...
                <p className="text-[10px] text-gray-500 mt-1">
                  技名は変更できます。使用時のセリフは技名に合わせたデフォルトが入り、自由に変更できます。
                </p>
              </div>
              ...
              {[0, 1, 2, 3].map((index) => (
                <div
                  key={index}
                  className="rounded-xl border border-pink-100 bg-pink-50/40 p-3 space-y-2"
                >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleSkillVoiceChange(index, e.target.value)
                      }
                      maxLength={100}
                      className="w-full px-3 py-2 border rounded-lg bg-white disabled:bg-gray-100"
                      placeholder={
                  ...
                  <div className="rounded-lg bg-white/80 border border-gray-100 px-3 py-2">
                    ...
                    <div className="text-[11px] text-gray-600 leading-relaxed">
                      {currentCoordinate?.skillDescriptions[index] ??
                        'コーデを選択すると表示されます。'}
                    </div>
                  </div>
                </div>
            ...
            <div className="pt-3 border-t border-gray-100">
              ...
              <p className="text-[10px] text-gray-500 mt-1">
                同じ端末では作成者トークンにより、次回から合言葉入力を省略できます。
              </p>
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 bg-pink-600 hover:bg-pink-700 disabled:bg-pink-300 disabled:cursor-wait text-white font-bold rounded-xl shadow"
            >
              {isModerating
                ? '安全確認中…'
                : editingId
            ...
            {!currentCoordinate ? (
              ...
              <>
                <div className="flex justify-between items-center">
                  <span className="font-bold px-2.5 py-1 rounded bg-pink-600 text-white text-xs">
                    {currentCoordinate.code.toUpperCase()}
                  </span>

                  <span className="text-xs font-bold text-gray-700">
                    {currentCoordinate.archetype}
                  </span>
                </div>
                ...
                <div className="text-xs bg-gray-50 border p-3 rounded-lg grid grid-cols-2 gap-2">
                  <div>
                    体力：<b>{currentCoordinate.stats.hp}</b>
                  </div>

                  <div>
                    知略：<b>{currentCoordinate.stats.intellect}</b>
                  </div>

                  <div>
                    特技：<b>{currentCoordinate.stats.charm}</b>
                  </div>

                  <div>
                    器用：<b>{currentCoordinate.stats.dexterity}</b>
                  </div>
                </div>
                ...
                <div className="text-xs bg-pink-50 border border-pink-100 p-3 rounded-lg space-y-3">
                  ...
                  {[0, 1, 2, 3].map((index) => {
                    const skillName =
                      customSkills[index] ||
                      currentCoordinate.defaultSkills[index];

                    const skillVoice =
                      skillVoices[index] ||
                      getDefaultSkillVoice(skillName);

                    return (
                      <div
                        key={index}
                        className="rounded-lg bg-white/80 border border-pink-100 p-2.5"
                      >
                        <div className="font-bold text-pink-700">
                          技{index + 1}：{skillName}
                        </div>
      ...
      <div className="pt-6 border-t border-gray-200 space-y-4">
        <div>
          ...
          <p className="text-[10px] text-gray-500 mt-1">
            この端末で作成したカードは、作成者トークンが残っているため自動的に編集・削除できます。
          </p>
        </div>

        {entries.filter((entry) => entry.cardType === 'coordinate').length ===
        0 ? (
          <p className="text-xs text-gray-500">
            まだエントリーされたキャラカードはありません。
          </p>
        ) : (
            ...
            {entries
              ...
              .map((entry) => {
                ...
                const canEdit = Boolean(
                  authorizedIds[entry.id] || creatorTokens[entry.id],
                );
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-pink-700 font-bold">
                          {preset
                            ? `${preset.code.toUpperCase()} / ${preset.name}`
                            : entry.presetId}
                        </div>
