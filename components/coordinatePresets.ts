...
const skillDescriptionsFor = (
  ...
  if (code === 'n1') {
    return [
      '自身のありのままの魅力を素直に押し出すスキル［全ステータス合計×10スコア］',
      '相手に合わせて自身の強みを見せつけるスキル［自身の任意のステータス−相手の対応ステータス）×40スコア］',
      '自身の魅せるスタイルを自在に変えられるスキル［自身の任意のステータス×10スコア&その基礎ステータスを倍にする、2回まで］',
      '自身の一番の魅力がより引き立つ状況を作るスキル［（使用時の三番目＋四番目ステータス）×10スコア&相手の全基礎ステータス×25%ダウン、1回のみ］',
    ];
  }
  ...
  return [
    '自身の一番の魅力を素直に押し出すスキル［基礎値一番目×20スコア］'.replace('基礎値一番目', `基礎値${STAT_LABELS[rank[0]]}`),
    '自身の二番目・三番目の魅力の掛け合わせに新しい価値を見出すスキル［基礎値二番目×三番目スコア］'.replace('基礎値二番目×三番目', `基礎値${STAT_LABELS[rank[1]]}×${STAT_LABELS[rank[2]]}`),
    '自身の一番の魅力で力量の差を見せつけるスキル［自身の基礎値一番目−相手の対応ステータス）×40スコア］'.replace('基礎値一番目', `基礎値${STAT_LABELS[rank[0]]}`),
    '自身の一番の魅力がより引き立つ状況を作るスキル［（基礎値三番目+四番目）×10スコア&自身の基礎値一番目に対応する相手のステータスを50%ダウン、1回のみ］'.replace('基礎値三番目+四番目', `基礎値${STAT_LABELS[rank[2]]}+${STAT_LABELS[rank[3]]}`).replace('基礎値一番目', `基礎値${STAT_LABELS[rank[0]]}`),
  ];
};
